...
const PredictionImage_1 = require("../models/PredictionImage");
const router = (0, express_1.Router)();
/**
 * POST /api/predictions/images
 * Guardar referencia de imagen para una predicción on-chain
 */
router.post('/images', auth_1.authenticateWallet, async (req, res) => {
    try {
        const { prediction_id_onchain, prediction_market_address, chain_id, image_url, image_path, tags } = req.body || {};
        if (!prediction_id_onchain || !prediction_market_address || !chain_id) {
            return res.status(400).json({
                error: 'prediction_id_onchain, prediction_market_address y chain_id son obligatorios',
            });
        }
        ...
        if (!image_url && (!tags || (Array.isArray(tags) && tags.length === 0))) {
            return res.status(400).json({
                error: 'Debe proporcionar al menos image_url o tags',
            });
        }
        ...
        const record = await PredictionImage_1.PredictionImageModel.create({
...
router.get('/:id/images', async (req, res) => {
    try {
        ...
        const predictionMarketAddress = req.query.prediction_market_address || '';
        const chainId = parseInt(req.query.chain_id || '31337', 10);
        if (!predictionMarketAddress) {
            return res.status(400).json({
                error: 'prediction_market_address es obligatorio',
            });
        }
        const record = await PredictionImage_1.PredictionImageModel.findByOnchainId(predictionIdOnchain, predictionMarketAddress, chainId);
        if (!record) {
